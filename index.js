...
function isValidBotInteraction(message, userState) {
  ...
  if (lowerMessage.startsWith('/debugorder ') || SHORTCUT_COMMANDS.includes(lowerMessage) || ['1', '2', '3', '4'].includes(message.trim())) {
    return true;
  }
...
// FIXED: Enhanced order matching function with stricter matching
function isOrderMatch(orderNumber, searchTerm) {
  const upperOrderNumber = orderNumber.toUpperCase();
  const upperSearchTerm = searchTerm.toUpperCase();
  
  // Split order number by common separators
  const orderParts = upperOrderNumber.split(/[-_\s#]/);
  
  // Check if any part starts with or equals the search term
  for (const part of orderParts) {
    if (part === upperSearchTerm || (part.startsWith(upperSearchTerm) && upperSearchTerm.length >= 3)) {
      return true;
    }
  }
...
app.listen(PORT, () => {
  ...
  console.log('✅ FIXED: Enhanced jacket order search with stricter matching');
  console.log('✅ FIXED: 1554 will no longer match F#1555Y-4-6, F1655Z-49, G54155Z-1-4');
